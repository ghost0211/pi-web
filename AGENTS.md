...
- Idle timeout: 10 minutes by default, configurable via `PI_WEB_SESSION_IDLE_TIMEOUT_MS` (`0` disables idle reaping). Concurrent `startRpcSession()` calls share a single start Promise (`globalThis.__piStartLocks`)
...
- Long turn rails keep the 15px pitch in a scrollable rail and render only bars near its viewport; wheel/keyboard navigation can reach all turns without squeezing them together or mounting thousands of buttons.
- Clicking a turn outside the loaded window calls `ensureEntryLoaded()` (`hooks/useAgentSession.ts`), which pages `.../context?before=` in 500-entry batches until the entry is present (without a fixed page-count limit); `ChatMinimap` then scrolls once the turn measures.
...
- `npm run desktop:server` assembles `src-tauri/{server,node}` (gitignored); it sets `PI_WEB_STANDALONE_BUILD=1`, which is the only thing that enables `output: "standalone"` in `next.config.ts` — npm release builds must keep producing regular `.next` output for `next start`. `pi-ai` deliberately uses variable imports for Node-only OAuth flows, so tracing must include both top-level and `pi-coding-agent`-nested `pi-ai/dist` trees; the build script validates every installed OAuth runtime against the source npm layout.
- `npm run desktop:build` produces the NSIS installer; CI (`.github/workflows/desktop-windows.yml`) builds it on `windows-latest` from `desktop-v*` tags. Every release increments the patch version and uses a new immutable tag; the workflow verifies that `desktop-vX.Y.Z`, `package.json`, and `src-tauri/Cargo.toml` agree. The desktop updater requires a stable public key in `tauri.conf.json` and the matching `TAURI_SIGNING_PRIVATE_KEY` Actions secret; CI publishes signed `.exe.sig` plus `desktop-latest.json` on a draft release before making that release latest. Never publish an unsigned updater feed or commit a private key.
