...
// the params through to both context/history builders (excludeLeaf on ?before).
...
test("context route parses ?tail and ?before, excluding the boundary on paging", () => {
  ...
  assert.match(routeSrc, /const targetLeafId = before \?\? leafId \?\? sm\.getLeafId\(\)/);
...
test("context route: ?before pages upward without duplicating the boundary", () => {
  ...
  const page1 = buildSessionHistory(entries, "e99", { tail: 5 }).entryIds;
  assert.deepEqual(page1, ["e95", "e96", "e97", "e98", "e99"]);
  const oldest = page1[0]; // e95
  const page2 = buildSessionHistory(entries, oldest, { tail: 5, excludeLeaf: true }).entryIds;
...
test("context route data reports when pagination reaches the root", () => {
  ...
  const page = buildSessionHistory(entries, "e0", { tail: 50, excludeLeaf: true });
