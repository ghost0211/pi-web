...
// parses ?tail (default 50, NaN-safe, capped at 1000) and feeds the same bounded
// options to the SDK context and raw UI history builders. The data-slicing behavior
// itself is covered end-to-end in lib/session-reader.pagination.test.mjs.
...
const { buildSessionHistory } = await jiti.import("@/lib/session-reader");
...
test("detail route parses ?tail: default 50, NaN-safe, capped at 1000", () => {
  ...
  assert.match(routeSrc, /const contextOptions = \{[^}]*tail,[^}]*sessionId: id[^}]*\}/);
  assert.match(routeSrc, /buildSessionContext\(entries as never, leafId, contextOptions\)/);
...
test("detail route bounds history to the tail window (default 50 over 5000 entries)", () => {
  ...
  const ctx = buildSessionHistory(entries, "e4999", { tail: 50 });
