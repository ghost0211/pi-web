...
test("queued callbacks from the old session cannot complete or disconnect its replacement", async () => {
  const { connection, sources, events } = createHarness();
  const oldReady = connection.ensureConnected("session-a");
  sources[0].open();
  ...
  await ready;
  lateMessage({ data: JSON.stringify({ type: "prompt_done" }) });
  lateError(new Event("error"));
  sources[1].message({ type: "agent_start" });
  assert.deepEqual(events.map(({ type }) => type), ["connected", "connected", "agent_start"]);
...
test("foreground refresh replaces a seemingly open stale stream", async () => {
  ...
  connection.refresh("session-a");
  assert.equal(sources[0].closeCount, 1);
  assert.equal(sources.length, 2);
  lateMessage({ data: JSON.stringify({ type: "prompt_done" }) });
  sources[1].open();
  sources[1].message({ type: "connected" });
  assert.deepEqual(events.map((event) => event.type), ["connected", "connected"]);
  connection.close();
});

test("foreground refresh preserves an in-progress readiness handshake", async () => {
