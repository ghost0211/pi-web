...
test("desktop settings show the already-running loopback server and optional tailnet forwarding", async () => {
  const settingsSource = await readFile(new URL("../components/SettingsPanel.tsx", import.meta.url), "utf8");
  const guide = await readFile(new URL("../desktop/README.md", import.meta.url), "utf8");
  assert.match(settingsSource, /window\.location\.origin/);
  assert.match(settingsSource, /tailscale serve --bg \{desktopWebPort\}/);
  assert.match(guide, /PI_WEB_ALLOWED_HOSTS/);
  assert.match(guide, /Never enable Tailscale Funnel/);
});
...
test("desktop opens web links in the system browser", async () => {
  ...
  // The app's own origin must never be treated as an external site, otherwise
  // the bundled loading page is pushed into the browser and cancelled. The URL
  // matrix itself is covered by the shell's Rust unit tests (run in CI).
  assert.match(source, /fn is_external_browser_url\(url: &Url\) -> bool \{[\s\S]*?!is_local_host\(url\.host_str\(\)\)/);
  assert.match(source, /host == "localhost" \|\| host\.ends_with\("\.localhost"\) \|\| host == "127\.0\.0\.1" \|\| host == "::1"/);
  assert.match(source, /fn the_app_origin_is_never_handed_to_the_system_browser\(\)/);
});

test("desktop reveals the window only after the app UI has loaded", async () => {
  const source = await readFile(new URL("../src-tauri/src/main.rs", import.meta.url), "utf8");
  // The loading page is bundled at `tauri.localhost`; creating the window hidden
  // is not enough because the readiness thread used to show it right after
  // navigating, which flashed that page on every launch.
  assert.match(source, /WebviewUrl::App\("index\.html"\.into\(\)\), false\)/);
  assert.match(
    source,
    /\.on_page_load\(\|window, payload\| \{[\s\S]*?payload\.event\(\) == PageLoadEvent::Finished && is_loopback_browser_url\(payload\.url\(\)\)[\s\S]*?window\.show\(\)/,
  );

  const readyBranch = source.slice(source.indexOf("if ready {"), source.indexOf("// On timeout keep"));
  assert.match(readyBranch, /window\.navigate\(url\)/);
  // A page that never finishes still has to reveal the window (with the
  // bundled loading page) instead of leaving the app invisible.
  assert.match(readyBranch, /std::thread::sleep\(UI_SHOW_FALLBACK\)/);
  assert.match(readyBranch, /if !fallback\.is_visible\(\)\.unwrap_or\(false\)/);

  const timeoutBranch = source.slice(source.indexOf("// On timeout keep"));
  assert.match(timeoutBranch, /let _ = window\.show\(\)/);
  assert.match(timeoutBranch, /let _ = window\.set_focus\(\)/);
  assert.match(source, /fn only_the_sidecar_origin_reveals_the_window\(\)/);
});
