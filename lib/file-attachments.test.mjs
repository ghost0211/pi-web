...
test("encodes a binary file as a labeled base64 block", () => {
  const output = encodeBinaryFileAttachment({
    name: "report.pdf",
    data: "SGVsbG8=",
    size: 5,
  });
  assert.equal(
    output,
    "[pi-web binary attachment: name=\"report.pdf\" size=\"5\" encoding=\"base64\"]\nSGVsbG8=\n[/pi-web binary attachment]",
  );
});

test("appends binary blocks after user text", () => {
  const output = buildMessageWithBinaryAttachments("解析这个文件", [{
    name: "data.bin",
    data: "AAEC",
    size: 3,
  }]);
  assert.equal(
    output,
    "解析这个文件\n\n[pi-web binary attachment: name=\"data.bin\" size=\"3\" encoding=\"base64\"]\nAAEC\n[/pi-web binary attachment]",
  );
});

test("keeps desktop-native attachments as absolute paths for trusted readers", () => {
  const path = "C:\\Users\\chentao\\Desktop\\稽核规则汇总-0903.xlsx";
  assert.equal(localAttachmentName(path), "稽核规则汇总-0903.xlsx");
  assert.equal(localAttachmentName("/tmp/report.pdf"), "report.pdf");
  assert.equal(
    buildMessageWithLocalFileAttachments("分析这个文件", [{ name: "稽核规则汇总-0903.xlsx", path }]),
    "分析这个文件\n\n[Local file attachment: 稽核规则汇总-0903.xlsx]\n"
      + `Absolute path: ${path}\n`
