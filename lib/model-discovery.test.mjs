...
test("resolves environment-backed headers without an API key", async (t) => {
  const agentDir = await mkdtemp(join(tmpdir(), "pi-web-discovery-auth-test-"));
  const previousDir = process.env.PI_CODING_AGENT_DIR;
  const previousToken = process.env.PI_WEB_DISCOVERY_TEST_TOKEN;
  t.after(async () => {
    if (previousDir === undefined) delete process.env.PI_CODING_AGENT_DIR;
    else process.env.PI_CODING_AGENT_DIR = previousDir;
    if (previousToken === undefined) delete process.env.PI_WEB_DISCOVERY_TEST_TOKEN;
    else process.env.PI_WEB_DISCOVERY_TEST_TOKEN = previousToken;
