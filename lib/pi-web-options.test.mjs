...
test("supports --help and -h without starting the server", () => {
  ...
  assert.match(getHelpText(), /--unsafe-no-auth/);
});
...
test("requires an explicit opt-out for unauthenticated remote access", () => {
  assert.equal(parseLaunchOptions(["--unsafe-no-auth"], {}).unsafeNoAuth, true);
  for (const value of ["1", "true", "TRUE", "yes", "on"]) {
    assert.equal(parseLaunchOptions([], { PI_WEB_UNSAFE_NO_AUTH: value }).unsafeNoAuth, true);
  }
});
