...
test("resolveSessionIdleTimeoutMs honors PI_WEB_SESSION_IDLE_TIMEOUT_MS", (t) => {
  const original = process.env.PI_WEB_SESSION_IDLE_TIMEOUT_MS;
  t.after(() => {
    if (original === undefined) delete process.env.PI_WEB_SESSION_IDLE_TIMEOUT_MS;
    else process.env.PI_WEB_SESSION_IDLE_TIMEOUT_MS = original;
  });

  delete process.env.PI_WEB_SESSION_IDLE_TIMEOUT_MS;
  assert.equal(resolveSessionIdleTimeoutMs(), 10 * 60 * 1000);

  process.env.PI_WEB_SESSION_IDLE_TIMEOUT_MS = "1800000";
  assert.equal(resolveSessionIdleTimeoutMs(), 30 * 60 * 1000);

  // 0 disables idle reaping entirely.
  process.env.PI_WEB_SESSION_IDLE_TIMEOUT_MS = "0";
  assert.equal(resolveSessionIdleTimeoutMs(), 0);

  // Malformed or negative values fall back to the default.
  process.env.PI_WEB_SESSION_IDLE_TIMEOUT_MS = "not-a-number";
  assert.equal(resolveSessionIdleTimeoutMs(), 10 * 60 * 1000);
  process.env.PI_WEB_SESSION_IDLE_TIMEOUT_MS = "-5";
  assert.equal(resolveSessionIdleTimeoutMs(), 10 * 60 * 1000);
});
...
test("reports an exact system prompt as effective without touching the SDK transcript", async (t) => {
  const inner = makePromptInner(() => Promise.resolve());
  ...
  // Pi owns the transcript's system message (AgentState.systemPrompt is
  // read-only since 0.87); pi-web forces its own text per turn through the
  // system-prompt extension instead of rewriting the SDK state.
  assert.equal(wrapper.getEffectiveSystemPrompt(), "context prompt");
  await wrapper.send({ type: "prompt", message: "hello" });
  await nextTurn();
  assert.equal(inner.agent.state.systemPrompt, "SDK base prompt");
  assert.equal(wrapper.getEffectiveSystemPrompt(), "context prompt");
});
