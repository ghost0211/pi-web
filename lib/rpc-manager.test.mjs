...
test("the wrapper forces system prompts through the extension boundary", async () => {
  const source = await readFile(new URL("./rpc-manager.ts", import.meta.url), "utf8");
  ...
  // Pi 0.87 owns the transcript's system prompt, so the prompt path must not
  // rewrite SDK state; the shared system-prompt extension does it per turn.
  assert.doesNotMatch(promptSource, /applyEffectiveSystemPrompt/);
  assert.doesNotMatch(source, /applyEffectiveSystemPrompt/);
  assert.doesNotMatch(source, /prepareNextTurnWithContext/);
  assert.doesNotMatch(promptSource, /requestedToolNames/);
  assert.match(source, /const systemPromptExtension = createSystemPromptExtension\(systemPromptState\)/);
  assert.match(source, /extensionFactories: \[\s*systemPromptExtension/);
  assert.match(source, /systemPromptState\.exact = exactSystemPrompt/);
  assert.match(source, /systemPrompt: this\.getEffectiveSystemPrompt\(\)/);
});
...
test("reload is rejected while the session is streaming", async () => {
  const source = await readFile(new URL("./rpc-manager.ts", import.meta.url), "utf8");
  const reloadSource = source.slice(
    source.indexOf('case "reload"'),
    source.indexOf('case "abort_compaction"'),
  );
  // Guards against the pi SDK wrapRegisteredTool race: an in-flight tool call
  // loses its real result to the stale-ctx assertion when reload lands mid-run.
  assert.match(reloadSource, /if \(this\.inner\.isStreaming\) \{\s*throw new Error\("Cannot reload while the session is running/);

  const actionsSource = source.slice(
    source.indexOf("reload: async () =>"),
    source.indexOf("private syncProjectTrust"),
  );
  assert.match(actionsSource, /if \(this\.inner\.isStreaming\) \{\s*throw new Error\("Cannot reload while the session is running/);
});
