...
test("the wrapper reapplies an exact prompt after SDK preflight", async () => {
  ...
  assert.match(promptSource, /preflightResult: \(success\) => \{[\s\S]*?this\.applyEffectiveSystemPrompt\(\);[\s\S]*?acceptPreflight\(\)/);
...
test("reloading a session invalidates the models cache", async () => {
  ...
  assert.match(reloadSource, /this\.applyEffectiveSystemPrompt\(\);\s*invalidateModelsCache\(\)/);
});
