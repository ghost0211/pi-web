...
test("buildSessionHistory exposes the first message parent for edit-from-here", () => {
  const entries = [
    { type: "model_change", id: "mc", parentId: null, provider: "test", modelId: "m", timestamp: "2026-01-01T00:00:00.000Z" },
    userEntry("u1", "mc", "first question"),
    assistantEntry("a1", "u1", "first answer"),
    userEntry("u2", "a1", "second question"),
  ];

  const history = buildSessionHistory(entries);
  assert.equal(history.firstEntryParentId, "mc");

  // A message entry at the branch root has no parent to navigate to.
  const rootless = buildSessionHistory([userEntry("u0", null, "only message")]);
  assert.equal(rootless.firstEntryParentId, null);
});
...
test("keeps compacted messages in chronological UI history", () => {
  const entries = [
    userEntry("u1", null, "old user request"),
    assistantEntry("a1", "u1", "old assistant answer"),
    userEntry("u2", "a1", "kept user request"),
  ...
  assert.deepEqual(history.entryIds, ["u1", "a1", "u2", "cmp", "u3"]);
  assert.deepEqual(
    history.messages.map((message) => [message.role, message.customType, message.content]),
    [
...
test("UI history can paginate past a compaction boundary", () => {
  const entries = [
    userEntry("u1", null, "old user request"),
    assistantEntry("a1", "u1", "old assistant answer"),
    userEntry("u2", "a1", "kept user request"),
  ...
  const latest = buildSessionHistory(entries, undefined, { tail: 3 });
  assert.deepEqual(latest.entryIds, ["u2", "cmp", "u3"]);
  assert.equal(latest.hasMore, true);

  const older = buildSessionHistory(entries, latest.oldestEntryId, { tail: 3, excludeLeaf: true });
  assert.deepEqual(older.entryIds, ["u1", "a1"]);
  assert.equal(older.hasMore, false);
});

test("UI history follows only the selected branch across compaction", () => {
  const entries = [
    ...
    userEntry("main", "cmp", "after compaction"),
    userEntry("alt", "a1", "alternate branch"),
  ];

  assert.deepEqual(buildSessionHistory(entries, "main").entryIds, ["u1", "a1", "u2", "cmp", "main"]);
  assert.deepEqual(buildSessionHistory(entries, "alt").entryIds, ["u1", "a1", "alt"]);
});

test("UI history retains every compaction marker", () => {
  const entries = [
    userEntry("u1", null, "first"),
    { type: "compaction", id: "cmp1", parentId: "u1", timestamp: "2026-01-01T00:00:01.000Z", summary: "summary 1", firstKeptEntryId: "u1", tokensBefore: 10 },
    userEntry("u2", "cmp1", "second"),
    { type: "compaction", id: "cmp2", parentId: "u2", timestamp: "2026-01-01T00:00:02.000Z", summary: "summary 2", firstKeptEntryId: "u2", tokensBefore: 20 },
    userEntry("u3", "cmp2", "third"),
  ];

  const history = buildSessionHistory(entries);
  assert.deepEqual(history.entryIds, ["u1", "cmp1", "u2", "cmp2", "u3"]);
  assert.deepEqual(
    history.messages.filter((message) => message.role === "custom").map((message) => message.content),
    ["summary 1", "summary 2"],
  );
});
