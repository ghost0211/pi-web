...
test("rejects files outside cwd, including symlink targets", async () => {
  ...
  try {
    ...
    // Directory junctions exercise realpath escape protection on Windows
    // without requiring administrator privileges for a file symlink.
    ...
    assert.throws(() => loadSubagentInputFiles(cwd, ["linked/article.md"]), /outside the session cwd/);
  } finally {
    await rm(cwd, { recursive: true, force: true });
