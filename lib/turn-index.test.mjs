...
test("maps trailing compaction turns to the chat end and does not borrow for unrendered messages", () => {
  const offsets = mapTurnOffsets([
    { top: 120 },
    { top: null, borrowNext: true },
    { top: null, borrowNext: true },
  ], 3, 900);
  assert.deepEqual([...offsets.entries()], [[0, 120], [1, 900], [2, 900]]);
  assert.deepEqual([...mapTurnOffsets([{ top: null, borrowNext: true }], 1, 900).entries()], [[0, 900]]);
  assert.equal(mapTurnOffsets([{ top: null, borrowNext: false }], 1, 900).size, 0);
});
